...
@dataclass
class ENCTranslationLevenshteinConfig(ENCTranslationConfig):
    ...
    pos_data: Optional[str] = field(
        default="data-bin/IWSLT_EN_VT/pos_s",
        metadata={
    ...
    pos_predict_data: Optional[str] = field(
        default="data-bin/IWSLT_EN_VT/pos_p",
        metadata={
            "help": "colon separated path to data directories list, will be iterated upon during epochs "
            "in round-robin manner; however, valid and test data are always in the first directory "
            "to avoid the need for repeating them in all directories",
...
@register_task("ENC_task", dataclass=ENCTranslationLevenshteinConfig)
class TranslationLevenshteinTask_v4(ENC_translation):
    """
    Translation (Sequence Generation) task for Levenshtein Transformer
    See `"Levenshtein Transformer" <https://arxiv.org/abs/1905.11006>`_.
    Added POS tagging model initialization and forward pass.
    """
    cfg: ENCTranslationLevenshteinConfig

    def __init__(self, cfg: ENCTranslationLevenshteinConfig, src_dict, tgt_dict, pos_dict, pos_predict_dict):
        super().__init__(cfg, src_dict, tgt_dict, pos_dict, pos_predict_dict)
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        pos_path = utils.split_paths(self.cfg.pos_data)
        pos_predict_path = utils.split_paths(self.cfg.pos_predict_data)
    ...
    def build_generator(self, models, args, **unused):
        ...
        from ..models.ENC.iterative_refinement_generator_ENC import IterativeRefinementGenerator
    ...
    def train_step(
        ...
        loss, sample_size, logging_output = criterion(
            model, sample, pos_model=self.pos_model)
        if ignore_grad:
            loss *= 0
