...
def create_conv_dict(model, penalty):
    ...
    if not os.path.isfile(f'./data-bin/IWSLT_EN_VT/conv_dicts/conv_dict_{penalty}.pkl'):
        ...
        # if not os.path.isfile(f'./data-bin/IWSLT_EN_VT/conv_dict_{penalty}.pkl'):
...
class IterativeRefinementGenerator(object):
    ...
    def generate(self, models, sample, prefix_tokens=None, constraints=None):
        ...
        encoder_ids = sample["pos"]
        pos_ids = sample["pos_predict"]
        ...
        pos_tags = model.pos_model.translate(src_sentences)
        pos_tags2 = model.pos_model.decode(
            encoder_ids[0])
        ...
        encoder_out = model.forward_encoder(
            src_tokens=src_tokens, src_lengths=src_lengths, encoder_ids=encoder_ids, pos_ids=pos_ids)
        ...
        for step in range(self.max_iter + 1):
            ...
            decoder_out = model.forward_decoder(
                prev_decoder_out, encoder_out, conv_dict=conv_dict, pos_sentences=pos_tag_sentences, pos_id=pos_ids, **decoder_options
            )
