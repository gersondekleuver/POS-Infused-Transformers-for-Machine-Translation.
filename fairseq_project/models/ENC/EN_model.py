...
def ensemble_encoder(func):
    def wrapper(self, src_tokens, src_lengths, encoder_ids=None, pos_ids=None, **kwargs):
        if self.ensemble_models is None or len(self.ensemble_models) == 1:
            return func(self, src_tokens, src_lengths, encoder_ids=encoder_ids, pos_ids=pos_ids, ** kwargs)
        encoder_outs = [
            func(model, src_tokens, src_lengths, encoder_ids=encoder_ids, pos_ids=pos_ids, **
...
def ensemble_decoder(func):
    def wrapper(self, normalize=False, encoder_out=None, encoder_ids=None, pos_ids=None, *args, **kwargs):
        if self.ensemble_models is None or len(self.ensemble_models) == 1:
            return func(
                self, normalize=normalize, encoder_out=encoder_out, encoder_ids=encoder_ids, pos_ids=pos_ids, *args, **kwargs
            )
        ...
        for i in range(len(action_outs[0])):
            if i == 0 and normalize:
                ...
                ensembled_outs += [torch.stack([a[i]
                                                for a in action_outs], -1)]
            else:
                ensembled_outs += [None]
...
class FairseqNATModel(TransformerModel):
    ...
    def forward_encoder(self, src_tokens, src_lengths, encoder_ids=None, pos_ids=None):
        return self.encoder(src_tokens=src_tokens, src_lengths=src_lengths, encoder_ids=encoder_ids, pos_ids=pos_ids)
