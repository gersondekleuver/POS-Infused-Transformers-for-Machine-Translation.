...
def create_conv_dict(model, penalty):

    # if not os.path.isfile(f'./data-bin/wmt14_en_de/conv_dict_{penalty}.pkl'):
    if not os.path.isfile(f'./data-bin/IWSLT_EN_VT/conv_dicts/conv_dict_{penalty}.pkl'):

        # file_dir = "data-bin/wmt14_en_de/"
        file_dir = "data-bin/IWSLT_EN_VT/bpe/"

        # files = ["test.en-de",
        #          "train.en-de", "valid.en-de"]
        ...
        for file in files:
            ...
            lang_text = read_txt(file_name + ".bpe.vt")
            pos_text = read_txt(file_name + ".bpe.pos.vt")

            for i in range(len(lang_text)):

                words = lang_text[i].split()

                pos_tags = pos_text[i].split()

                for j in range(len(pos_tags)):

                    try:
                        if not word2pos_dict.get(words[j]):

                            word2pos_dict[words[j]] = set()

                    except:
                        count += 1
                        continue

                    word2pos_dict[words[j]].add(pos_tags[j])
                    pos_set.add(pos_tags[j])
                    word_set.add(words[j])

        for pos_tag in pos_set:

            conv_dict[pos_tag] = torch.zeros(
                len(model.decoder.dictionary.symbols))

        for i, word in enumerate(model.decoder.dictionary.symbols):

            if word2pos_dict.get(word):
                pos_tags = word2pos_dict[word]

                for pos_tag in pos_tags:

                    conv_dict[pos_tag][i] = penalty

        # with open(f'./data-bin/wmt14_en_de/conv_dict_{penalty}.pkl', 'wb') as f3:

        with open(f'./data-bin/IWSLT_EN_VT/conv_dicts/conv_dict_{penalty}.pkl', 'wb') as f3:
            pickle.dump(conv_dict, f3)

    else:
        # with open(f'./data-bin/wmt14_en_de/conv_dict_{penalty}.pkl', 'rb') as f3:

        with open(f'./data-bin/IWSLT_EN_VT/conv_dicts/conv_dict_{penalty}.pkl', 'rb') as f3:
            conv_dict = pickle.load(f3)
...
class IterativeRefinementGenerator(object):
    ...
    def generate(self, models, sample, prefix_tokens=None, constraints=None):
        ...
        inference_step_args = {}
        results = []
        for batch in pos_model._build_batches(tokenized_sentences, False):
        ...
        pos_out = [hypos for _, hypos in sorted(results, key=lambda x: x[0])]

        ids_out = [hypos[0]["tokens"]
                   for hypos in pos_out]
        # create a tensor matrix of pos ids

        pos_ids = torch.zeros(
            (len(ids_out), max([len(ids) for ids in ids_out]))).long().to(device)

        for i, ids in enumerate(ids_out):
            pos_ids[i, :len(ids)] = ids

        pos_tags = [pos_model.decode(hypos[0]["tokens"]) for hypos in pos_out]

        pos_tag_sentences = []
        penalty = model.pos_model.penalty
        ...
        # check if postag_dict file exits
        conv_dict = create_conv_dict(model, penalty)
        ...
        prev_decoder_out = model.initialize_output_tokens(
            encoder_out, src_tokens, length_tgt)
        ...
        if self.beam_size > 1:
            ...
            length_beam_order = (
                utils.new_arange(src_tokens, self.beam_size,
                                 bsz).t().reshape(-1)
            )
            ...
            prev_decoder_out = model.regenerate_length_beam(
                prev_decoder_out, self.beam_size, length_tgt
            )
        ...
        def is_a_loop(x, y, s, a):
            ...
            if l_x > l_y:
                y = torch.cat(
                    [y, x.new_zeros(b, l_x - l_y).fill_(self.pad)], 1)
                s = torch.cat([s, s.new_zeros(b, l_x - l_y)], 1)
                if a is not None:
                    a = torch.cat([a, a.new_zeros(b, l_x - l_y, a.size(2))], 1)
            elif l_x < l_y:
                x = torch.cat(
                    [x, y.new_zeros(b, l_y - l_x).fill_(self.pad)], 1)
        ...
        for step in range(self.max_iter + 1):
            ...
            decoder_out = model.forward_decoder(
                prev_decoder_out, encoder_out, conv_dict=conv_dict, pos_sentences=pos_tag_sentences, pos_ids=pos_ids, **decoder_options
            )
            ...
            if self.retain_history:
                finalized_history_tokens = [h[terminated]
                                            for h in decoder_out.history]
            ...
            # terminate the pos tag sentences that are terminated
            new_pos_tags = []
        ...
        if self.beam_size > 1:
            if reranker is not None:
                finalized = self.rerank(
                    reranker, finalized, [src_tokens,
                                          src_lengths], self.beam_size
                )
    ...
    def rerank(self, reranker, finalized, encoder_input, beam_size):
        ...
        length_beam_order = (
            utils.new_arange(
                final_output_tokens, beam_size, reranker_encoder_out.encoder_out.size(
                    1)
            )
            .t()
            .reshape(-1)
        ...
        reranking_scores = reranker.get_normalized_probs(
            reranker.decoder(
                final_output_tokens[:, :-1], reranker_encoder_out),
            True,
            None,
        )
